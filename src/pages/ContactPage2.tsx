...
const serviceOptions = [
  ...
  "Event / Activation", "Digital Experience", "Creative Communication",
...
const contactSchema = {
  ...
  "@graph": [
    ...
    { "@type": "ProfessionalService", name: "ØRIONS", url: "https://orions.agency/contact", telephone: "+66-92-390-5464", email: "hello@orions.agency",
      address: { "@type": "PostalAddress", streetAddress: "246/8 Soi Yothinpattana", addressLocality: "Khlong Chan, Bang Kapi", addressRegion: "Bangkok", postalCode: "10240", addressCountry: "TH" },
    },
  ],
};
...
const ContactPage2 = () => {
  ...
  return (
    <main className="bg-background min-h-screen grain-overlay">
      <SEO title="Contact — ØRIONS" description="Ready to change how people see your brand? Let's talk. Bangkok-based creative agency." path="/contact" schema={contactSchema} />
      <Navbar />
            ...
            <AnimatedSection>
              <div className="space-y-12">
                ...
                <div>
                  ...
                  <p className="font-body text-[15px] leading-[1.8] text-muted-foreground">246/8 Soi Yothin Pattana<br />Khlong Chan, Bang Kapi<br />Bangkok 10240, Thailand</p>
                </div>
                <div>
                ...
                <div className="flex items-center gap-8 pt-4">
                  {[
                    ...
                    <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground/50 hover:text-accent-warm transition-colors duration-300">{social.name}</a>
                  ))}
                </div>
              </div>
            </AnimatedSection>
              ...
              {submitted ? (
                <div className="flex items-center justify-center h-full min-h-[400px]">
                  <div className="text-center">
                    <p className="font-display text-[36px] text-accent-gradient mb-4">Thank you.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  <div>
                    ...
                    <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-transparent border-b border-border py-3 font-body text-[15px] text-foreground focus:outline-none focus:border-accent-warm transition-colors duration-300 placeholder:text-muted-foreground/30" placeholder="Your name" />
                  </div>
                  <div>
                    <label className="font-mono text-[10px] tracking-[0.15em] uppercase text-muted-foreground/60 block mb-3">Email</label>
                    <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full bg-transparent border-b border-border py-3 font-body text-[15px] text-foreground focus:outline-none focus:border-accent-warm transition-colors duration-300 placeholder:text-muted-foreground/30" placeholder="your@email.com" />
                  </div>
                  <div>
                    <label className="font-mono text-[10px] tracking-[0.15em] uppercase text-muted-foreground/60 block mb-3">Service</label>
                    <select value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} className="w-full bg-transparent border-b border-border py-3 font-body text-[15px] text-foreground focus:outline-none focus:border-accent-warm transition-colors duration-300 appearance-none cursor-pointer">
                      <option value="" className="bg-background text-muted-foreground">Select a service</option>
                      {serviceOptions.map((s) => (<option key={s} value={s} className="bg-background text-foreground">{s}</option>))}
                    </select>
                  </div>
                  <div>
                    <label className="font-mono text-[10px] tracking-[0.15em] uppercase text-muted-foreground/60 block mb-3">Message</label>
                    <textarea required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="w-full bg-transparent border-b border-border py-3 font-body text-[15px] text-foreground focus:outline-none focus:border-accent-warm transition-colors duration-300 resize-none placeholder:text-muted-foreground/30" placeholder="Tell us about your project" />
                  </div>
                  <button type="submit" className="font-mono text-[11px] tracking-[0.12em] uppercase text-primary-foreground bg-primary px-10 py-3.5 hover:bg-accent-warm hover:text-accent-warm-foreground transition-all duration-300 w-full mt-4">Send Message →</button>
                </form>
              )}
            </AnimatedSection>
          </div>
