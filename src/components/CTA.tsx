...
const CTA = ({ to, href, variant = "primary", children, className = "", external, noMagnetic }: Props) => {
  ...
  const base =
    variant === "primary"
      ? "group relative inline-flex items-center gap-3 bg-foreground text-background px-7 py-4 rounded-full btn-label overflow-hidden border border-foreground transition-colors duration-500 hover:text-foreground"
      : variant === "invert"
      ? "group relative inline-flex items-center gap-3 bg-background text-foreground px-7 py-4 rounded-full btn-label overflow-hidden border border-background transition-colors duration-500 hover:text-background"
